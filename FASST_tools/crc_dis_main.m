...
handles.indexMEEG   =   fliplr(intersect(indchantype(handles.Dmeg{1},{'EEG','MEG'}),handles.index))';
...
handles.indeeg = indchantype(D,{'EEG','LFP'});
% Default values
...
handles.chanlab = indchantype(D,{'EEG','MEG'});
...
[B,A] = butter(forder,[handles.filter.other(1)/(fsample(handles.Dmeg{i})/2),...
    handles.filter.other(2)/(fsample(handles.Dmeg{i})/2)]);
handles.filter.coeffother=[B;A];
...
[B,A] = butter(forder,[handles.filter.EMG(1)/(fsample(handles.Dmeg{i})/2),...
    handles.filter.EMG(2)/(fsample(handles.Dmeg{i})/2)]);
handles.filter.coeffEMG=[B;A];
...
[B,A] = butter(forder,[handles.filter.EOG(1)/(fsample(handles.Dmeg{i})/2),...
    handles.filter.EOG(2)/(fsample(handles.Dmeg{i})/2)]);
handles.filter.coeffEOG=[B;A];
guidata(hObject, handles);
...
if handles.figz~=0 && isvalid(handles.figz)
    z   =   handles.figz;
else
    z   =   figure;
end
...
indexMEGPLAN = intersect(indchantype(handles.Dmeg{1}, 'MEGPLANAR'), handles.index);
if norm
    n = 0;
...
if handles.displevt
    ...
    elseif sum(difdur)==0 && ~isempty(handles.chosevt)
        handles.displevt=handles.chosevt(1);
    elseif isempty(handles.chosevt)
        disp('No events to display')
        return
    else
...
try
    ...
    namescor = inputdlg(prompt,dlg_title,num_lines,def);
    if ~isempty(namescor)
        handles.score(2,1) = namescor;
    else
        % cancel
        return
    end
    ...
    scorsiz = inputdlg(prompt,dlg_title,num_lines,def);
    if ~isempty(scorsiz)
        handles.score{3,1} = str2double(scorsiz);
    end
...
for i=1:maxi
    ...
    if ~nosign  %if multiple comparison and no signal in the considered file
        ...
        for j=1:maxj
            ...
            if  any(index(j)==indchantype(handles.Dmeg{i},'EMG'))  %strfind(testcond{:},'EMG')
            ...
            elseif any(index(j)==indchantype(handles.Dmeg{i},'EOG')) %strfind(testcond{:},'EOG')
            ...
            elseif any(index(j)==indchantype(handles.Dmeg{i},'ECG')) %strfind(testcond{:},'ECG'
            ...
            elseif any(index(j)==indchantype(handles.Dmeg{i},{'EEG','MEG'})) %'EEG', 'MEGMAG', MEGPLANAR'               
           ...
           %Plot data 
           % bad channel?
           badchans = badchannels(handles.Dmeg{i});
           if any(ismember(badchans,index(j)))
               linetype = '--';
           else
               linetype = '-';
           end
            ...
            switch  selchan
                ...
                case 'MEG'
                    ...
                    if  strcmpi(chtyp,'MEGMAG') 
                        plt     =   plot(temps,factscale+(handles.Dmeg{i}(index(j),toshow))/scal,linetype,'Color',[0.25 0.25 0.25]);
                    elseif  strcmpi(chtyp,'MEGPLANAR')
                        if (~normalize)
                            plt     =   plot(temps,factscale+(handles.Dmeg{i}(index(j),toshow))/scal,linetype,'Color',[0.5 0.5 0]);
                        else
                            plt     =   plot(temps,factscale+(((handles.Dmeg{i}(index(j),toshow)).^2+...
                                        (handles.Dmeg{i}(index(j)+1,toshow)).^2).^0.5)/scal,linetype,'Color',[0 0.5 0]);
                        end
                    end
                 case    'REF1'
                    plt         = 	plot(temps,factscale + (handles.Dmeg{i}(index(j),toshow))/scal,linetype);             
                 case    'MEAN OF REF'
                    ...
                    plt         =   plot(temps,factscale+(toplotdat)/scal,linetype);
                ...
                case    'BIPOLAR'
                    ...
                    if ~isempty(index3)
                        ...
                        plt  	=   plot(temps,factscale+(bipolar)/scal,linetype,'Color','k');
                    else
                        plt     =   plot(temps,factscale+(handles.Dmeg{i}(index(j),toshow))/scal,linetype,'Color','k');       
                    end
            ...
            if plt~=0 && any(index(j)==indchantype(handles.Dmeg{i},'EOG'))
...
[B,A] = butter(forder,[frqcut(1)/(fsample(handles.Dmeg{ii})/2),...
    frqcut(2)/(fsample(handles.Dmeg{ii})/2)]);
